  ...
  const aggregatedData = React.useMemo(() => {
    ...
    rawData
      ...
      .forEach((entry) => {
        ...
        const key = entry.tournamentId;

        if (!groupedData[key]) {
          groupedData[key] = {
            winner: "N/A",
            winnerCommanders: {},
            winnerCommanderColorIdentity: "N/A",
            totalEntries: 0,
          };
        }

        const tournamentData = groupedData[key];

        // Increment commander counts
        tournamentData.commanderCounts[commanderPair] = (tournamentData.commanderCounts[commanderPair] || 0) + 1;

        // Update the most recent date for this commander pair
        tournamentData.recentCommanderDates[commanderPair] = Math.max(
          tournamentData.recentCommanderDates[commanderPair] || 0,
        ...
        // Update player stats
        tournamentData.totalEntries += 1;
        tournamentData.totalWins += entry.wins;
        tournamentData.totalLosses += entry.losses;
        tournamentData.totalDraws += entry.draws;
        tournamentData.totalGames += entry.wins + entry.losses + entry.draws;

        if (entry.standing === 1) tournamentData.top1 += 1;
        if (entry.standing <= 4) tournamentData.top4 += 1;
        if (entry.standing <= entry.topCut) tournamentData.topCut += 1;
      });
